...
exports.edit = async (client, message, args, data) => {
	...
	await sql.get(`UPDATE settings SET starboard = "${channel.id}" WHERE guild = "${message.guild.id}"`);
...
exports.delete = async (client, message, args, data) => {
	...
	await sql.get(`UPDATE settings SET starboard = null WHERE guild = ${message.guild.id}`);
