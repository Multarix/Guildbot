...
exports.edit = async (client, message, args, data, info) => {
	...
	await sql.run(`UPDATE settings SET ${info.sqlName} = "${roleMention.id}" WHERE guild = "${message.guild.id}"`);
...
exports.delete = async (client, message, args, data, info) => {
	...
	await sql.run(`UPDATE settings SET ${info.sqlName} = null WHERE guild = "${message.guild.id}"`);
